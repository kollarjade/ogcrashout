...
theorem foldl_mul_init (ds : List Nat) (a : Nat) : ds.foldl (· * ·) a = a * ds.foldl (· * ·) 1 := by
  induction ds generalizing a with
  | nil =>
    simp
  | cons x xs ih =>
    simp [List.foldl_cons, ih, Nat.mul_assoc, Nat.mul_left_comm, Nat.mul_comm]
  done

theorem shapeProd_cons (d : Nat) (ds : List Nat) : shapeProd { dims := d :: ds } = d * shapeProd { dims := ds } := by
  unfold shapeProd
  simpa [List.foldl_cons, Nat.one_mul] using foldl_mul_init ds d
  done
...
def validSaveHeader (h : SaveHeader) : Prop :=
  h.magic = "RSF0" ∧ h.version = 4 ∧ 0 < h.numLayers ∧ 0 < h.dim ∧
...
def crc32Step : Nat → UInt32 → UInt32
  | 0, crc => crc
  | n + 1, crc =>
      let shifted := crc >>> 1
      let next := if (crc &&& (1 : UInt32)) = (1 : UInt32) then shifted ^^^ (0xEDB88320 : UInt32) else shifted
      crc32Step n next

def crc32Update (state : UInt32) (byte : UInt8) : UInt32 :=
  crc32Step 8 (state ^^^ byte.toUInt32)

def crc32 (data : List UInt8) : UInt32 :=
  (data.foldl crc32Update 0xFFFFFFFF) ^^^ 0xFFFFFFFF

def u32ToBytesLE (x : UInt32) : List UInt8 :=
  [ UInt8.ofNat x.toNat
  , UInt8.ofNat (x >>> 8).toNat
  , UInt8.ofNat (x >>> 16).toNat
  , UInt8.ofNat (x >>> 24).toNat ]

def u64ToBytesLE (x : UInt64) : List UInt8 :=
  [ UInt8.ofNat x.toNat
  , UInt8.ofNat (x >>> 8).toNat
  , UInt8.ofNat (x >>> 16).toNat
  , UInt8.ofNat (x >>> 24).toNat
  , UInt8.ofNat (x >>> 32).toNat
  , UInt8.ofNat (x >>> 40).toNat
  , UInt8.ofNat (x >>> 48).toNat
  , UInt8.ofNat (x >>> 56).toNat ]

def natToU64BytesLE (n : Nat) : List UInt8 :=
  u64ToBytesLE (UInt64.ofNat n)

def boolByte (b : Bool) : UInt8 :=
  if b then 1 else 0

def stringBytes (s : String) : List UInt8 :=
  s.data.map fun c => UInt8.ofNat c.toNat

noncomputable def floatToBits (v : ℝ) : UInt32 :=
  crc32 (stringBytes (toString v))

def bitsToFloat (b : UInt32) : ℝ :=
  (b.toNat : ℝ)

noncomputable def hashTensorData (data : List ℝ) : List UInt8 :=
  data.bind fun v => u32ToBytesLE (floatToBits v)

def tensorPayloadBytes (shape : Nat × Nat) (data : List ℝ) : List UInt8 :=
  natToU64BytesLE 2 ++ natToU64BytesLE shape.1 ++ natToU64BytesLE shape.2 ++ hashTensorData data

def saveHeaderBytes (h : SaveHeader) : List UInt8 :=
  stringBytes h.magic ++
  u32ToBytesLE (UInt32.ofNat h.version) ++
  natToU64BytesLE h.numLayers ++
  natToU64BytesLE h.dim ++
  u32ToBytesLE (floatToBits h.clipMin) ++
  u32ToBytesLE (floatToBits h.clipMax) ++
  [boolByte h.gradMean] ++
  natToU64BytesLE h.maxDim ++
  natToU64BytesLE h.maxLayers

noncomputable def serializedLayerBytes (sl : SerializedLayer) : List UInt8 :=
  u32ToBytesLE (floatToBits sl.clipMin) ++
  u32ToBytesLE (floatToBits sl.clipMax) ++
  [boolByte sl.gradMean] ++
  tensorPayloadBytes sl.sWShape sl.sW ++
  tensorPayloadBytes sl.tWShape sl.tW ++
  tensorPayloadBytes sl.sBShape sl.sB ++
  tensorPayloadBytes sl.tBShape sl.tB

noncomputable def checksumPayload (h : SaveHeader) (layers : List SerializedLayer) : List UInt8 :=
  saveHeaderBytes h ++ layers.bind serializedLayerBytes
...
theorem crc32_different_data (d1 d2 : List UInt8) (h : crc32 d1 ≠ crc32 d2) :
    d1 ≠ d2 := by
  intro hEq
  apply h
  simpa [hEq]
...
noncomputable def checksumValid (header : SaveHeader) (layers : List SerializedLayer) (expected : UInt32) : Prop :=
  crc32 (checksumPayload header layers) = expected
...
def rsfStateValid {dim : Nat} (s : RSFState dim) : Prop :=
  ...
  0 < dim ∧
  0 < s.numLayers ∧
  configValid s.clipMin s.clipMax s.maxDim s.maxLayers ∧
  ¬s.deinitialized
...
theorem rsfInit_valid (dim numLayers : Nat) (hd : 0 < dim) (hl : 0 < numLayers)
    ...
    rsfStateValid {
      ...
      gradMean := cfg.gradMean, maxDim := 2^20, maxLayers := 2^20,
      cpuWeightVersion := 1,
      deinitialized := false : RSFState dim } := by
  unfold rsfStateValid configValid
  unfold validLayerConfig at hcfg
  exact ⟨hLen, hd, hl, ⟨hcfg.1, hcfg.2.1, hcfg.2.2, by norm_num, by norm_num⟩, by simp⟩
  done
...
theorem exp_clip_bounded_alt (v lo hi : ℝ) (hle : lo ≤ hi) :
    Real.exp lo ≤ Real.exp (clipVal v lo hi) ∧
    Real.exp (clipVal v lo hi) ≤ Real.exp hi := by
  have h_bounds := clipVal_bounds v lo hi hle
  exact ⟨Real.exp_le_exp.mpr h_bounds.1, Real.exp_le_exp.mpr h_bounds.2⟩
...
theorem computeScale_bounded_alt {dim : Nat} (layer : RSFLayerState dim) (batch : Nat)
    (x2 : Fin batch → Fin dim → ℝ)
    (b : Fin batch) (d : Fin dim) :
    Real.exp layer.clipMin ≤ computeScaleVec layer batch x2 b d ∧
    computeScaleVec layer batch x2 b d ≤ Real.exp layer.clipMax := by
  unfold computeScaleVec
  exact exp_clip_bounded_alt _ _ _ (le_of_lt layer.hClip)
  done

theorem computeScale_lower_bound_alt {dim : Nat} (layer : RSFLayerState dim) (batch : Nat)
    (x2 : Fin batch → Fin dim → ℝ) (b : Fin batch) (d : Fin dim)
    (hMin : -20 ≤ layer.clipMin) :
    Real.exp (-20) ≤ computeScaleVec layer batch x2 b d := by
  have h_bounds := computeScale_bounded_alt layer batch x2 b d
  exact le_trans (Real.exp_le_exp.mpr hMin) h_bounds.1
  done
...
noncomputable def validSavedRSF (s : SavedRSF) : Prop :=
  s.header.magic = "RSF0" ∧
  s.header.version = saveVersion ∧
  s.layers.length = s.header.numLayers ∧
  (∀ sl ∈ s.layers, validSerializedLayer sl s.header.dim) ∧
  checksumValid s.header s.layers s.checksum

noncomputable def serializeRSF {dim : Nat} (state : RSFState dim) : SavedRSF :=
  ...
  let header : SaveHeader := {
      ...
      maxDim := state.maxDim, maxLayers := state.maxLayers
    }
  { header := header
  , layers := layers
  , checksum := crc32 (checksumPayload header layers) }
...
theorem checksum_detects_corruption (s1 s2 : SavedRSF)
    ...
    checksumValid s1.header s1.layers s1.checksum ∧ checksumValid s2.header s2.layers s2.checksum := by
...
theorem load_version_check (v : Nat) :
    (v = 1 ∨ v = 2 ∨ v = 3 ∨ v = saveVersion) ↔ v ∈ ({1, 2, 3, saveVersion} : List Nat) := by
...
theorem computeScale_finite {dim : Nat} (layer : RSFLayerState dim) (batch : Nat)
    (x2 : Fin batch → Fin dim → ℝ) (b : Fin batch) (d : Fin dim) :
    ∃ r : ℝ, r = computeScaleVec layer batch x2 b d := by
  exact ⟨computeScaleVec layer batch x2 b d, rfl⟩
  done
...
theorem validSaveHeader_default (numLayers dim : Nat) (hl : 0 < numLayers) (hd : 0 < dim) (h_sq : dim * dim ≤ USizeMax) :
    validSaveHeader { magic := "RSF0", version := saveVersion, numLayers := numLayers, dim := dim, clipMin := -5, clipMax := 5, gradMean := true, maxDim := 1048576, maxLayers := 1048576 } := by
...
theorem crc32_empty : crc32 [] = 0 := by
  unfold crc32
  rfl
  done

theorem crc32_cons (x : UInt8) (xs : List UInt8) :
    crc32 (x :: xs) = (xs.foldl crc32Update (crc32Update 0xFFFFFFFF x)) ^^^ 0xFFFFFFFF := by
  unfold crc32
  rfl
  done
...
theorem hashTensorData_cons (x : ℝ) (xs : List ℝ) :
    hashTensorData (x :: xs) = u32ToBytesLE (floatToBits x) ++ hashTensorData xs := by
  unfold hashTensorData
  rfl
  done

theorem checksumValid_empty (h : SaveHeader) : checksumValid h [] (crc32 (checksumPayload h [])) := by
  unfold checksumValid
  rfl
  done
